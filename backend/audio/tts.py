    ...
    if language_lower in ["zh", "cmn", "zho", "chinese", "mandarin", "zh-cn"]:
        if _HAS_PYKOKORO:
            try:
                ...
                # Synthesize with PyKokoro.
                # Split Chinese text by punctuation and synthesize raw content only.
                # Using punctuation-bearing chunks can trigger repetition/stutter in some cases.
                segments = re.split(r'[，。！？；、：,.!?;:]+', text)
                text_segments = [_remove_immediate_cjk_repetition(s.strip()) for s in segments if s.strip()]
                
                # If no segments found (no punctuation), use original text
                if not text_segments:
                    text_segments = [_remove_immediate_cjk_repetition(text)]
                ...
                for segment in text_segments:
                    if not segment.strip():
                        continue
                    result = _pykokoro_cache_zh.run(
                        _remove_immediate_cjk_repetition(segment.strip()),
                        generation=GenerationConfig(lang='zh')
                    )
                    ...
                    # Insert a short pause between chunks to keep natural boundaries.
                    if sample_rate and len(seg_audio) > 0:
                ...
                if audio_segments:
                    ...
                    result = _pykokoro_cache_zh.run(
                        _remove_immediate_cjk_repetition(text),
                        generation=GenerationConfig(lang='zh')
                    )
                    audio_array = result.audio
                    sampling_rate = result.sample_rate
